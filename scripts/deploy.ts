...
const main = async () => {
  ...
  const EURMToken = await ethers.getContractFactory("ERC20");
	...
	console.log("EURM Owner Address:", await eurmToken.owner());
	console.log("Balance of the Owner: ", (await eurmToken.balanceOf(await accounts[0].getAddress())).toString(), "must be 100 billion!!! in wei");
	console.log("Total Supply: ", (await eurmToken.totalSupply()).toString(), "must be 100 billion!!! in wei");
	// Try to mint one additional Token
	console.log("============= Try to Mint Any Additional Token (Expect Revert) =================");
	try {
		const estimatetx = await eurmToken.mint(1, {gasLimit: 1500000});
